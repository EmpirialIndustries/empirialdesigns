...
const About = () => {
  ...
  return (
    <section id="about" className="py-24 bg-white/95 backdrop-blur-lg relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-20">
          {workflowSteps.map((step, index) => (
            <div 
              key={step.step}
              className="bg-black/80 backdrop-blur-lg border border-white/20 rounded-3xl p-10 text-center hover:border-white/40 transition-all duration-500 group hover:scale-105 elegant-shadow animate-fade-in"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-20">
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={index} className="flex items-start group">
                  ...
                  <span className="text-gray-600 text-lg font-medium">{point}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 animate-fade-in" style={{ animationDelay: '0.4s' }}>
            {values.map((value, index) => (
              <div 
                key={value.title}
                className="bg-black/5 backdrop-blur-lg border border-black/10 rounded-3xl p-10 hover:border-black/30 transition-all duration-500 group elegant-shadow"
              >
